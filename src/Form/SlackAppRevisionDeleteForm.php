...
/**
 * Provides a form for deleting a Slack App revision.
 *
 * @ingroup slack
 */
class SlackAppRevisionDeleteForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'slack_app_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->getLogger('content')->notice('Slack App: deleted %title revision %revision.', [
    ...
    if ($this->connection->query('SELECT COUNT(DISTINCT vid) FROM {slack_app_field_revision} WHERE id = :id', [':id' => $this->revision->id()])->fetchField() > 1) {
      $form_state->setRedirect(
